...
namespace ActionDesigner.Editor
{
    public class UIToolkitNodeInspector : VisualElement
    {
        ...
        public void UpdateSelection(NodeView nodeView)
        {
            ...
            // ActionRunner는 Selection에서 찾기
            var gameObject = Selection.activeGameObject;
            ActionRunner actionRunner = null;
            if (gameObject != null)
            {
                actionRunner = gameObject.GetComponent<ActionRunner>();
            }

            if (actionRunner == null)
            {
                ShowError("ActionRunner not found. Please select a GameObject with ActionRunner component.");
                return;
            }

            ShowInspector(nodeView, actionRunner);
        }
